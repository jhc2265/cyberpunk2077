...
:gh_makerepo
echo.
echo   1) Use an EXISTING repo from your GitHub account
echo   2) Create a NEW repo
set /p GHREPOMODE="Choose 1 or 2: "
echo.
if "!GHREPOMODE!"=="1" goto gh_existing_repo
goto gh_newrepo

:gh_existing_repo
echo Your existing GitHub repos:
gh repo list --limit 100
echo.
set /p REPONAME="Type the repo to use (e.g. cafe24 or owner/cafe24): "
if "!REPONAME!"=="" goto end
set "REPOURL="
for /f "delims=" %%U in ('gh repo view "!REPONAME!" --json url --jq .url 2^>nul') do set "REPOURL=%%U"
if not defined REPOURL (
  echo.
  echo [x] Could not find that repo on your account. Check the name and try again.
  echo.
  goto gh_existing_repo
)
echo Connecting to: !REPOURL!
call :gitinit
...
echo.
echo Which branch should the SITE be published to?
echo   1) main
echo   2) An EXISTING branch ^(I'll show the list^)
echo   3) A NEW branch name I'll type
echo   ^(just press Enter to use the default: gh-pages^)
set /p PUBMODE="Choose 1 / 2 / 3: "
set "PUBBRANCH="
if "!PUBMODE!"=="1" set "PUBBRANCH=main"
if "!PUBMODE!"=="2" goto pick_existing_branch
if "!PUBMODE!"=="3" goto pick_new_branch
if not defined PUBBRANCH set "PUBBRANCH=gh-pages"
goto do_publish

:pick_existing_branch
echo.
echo Branches that already exist on GitHub ^(use the name shown after "refs/heads/"^):
git ls-remote --heads origin
echo.
set /p PUBBRANCH="Type the branch name (e.g. main): "
if "!PUBBRANCH!"=="" set "PUBBRANCH=gh-pages"
goto do_publish

:pick_new_branch
echo.
set /p PUBBRANCH="Type a NEW branch name (e.g. site, deploy): "
if "!PUBBRANCH!"=="" set "PUBBRANCH=gh-pages"
goto do_publish

:do_publish
echo.
echo Publishing folder "!PUBDIR!" to branch "!PUBBRANCH!" ...
call npx gh-pages -d "!PUBDIR!" -b "!PUBBRANCH!"
if defined FIRSTTIME (
  echo.
  echo [First time only] Finish on GitHub:
  echo   Settings ^> Pages ^> Source ^> select "!PUBBRANCH!" branch ^> Save.
